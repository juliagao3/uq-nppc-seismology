...
class RestorationModel(object):
    def __init__(
        ...
        elif distortion_type == 'inpainting_3':
            ## Inpainting mouses in CelebA 256x256 images
            mask = gen_mask(self.x_shape, 165, 250, 50, 200).to(self.device)
            self.distortion_model = Inpainting(mask=mask, fill=self.data_module.mean).to(self.device)
...
class RestorationTrainer(object):
    def __init__(
        ...
        self.fixed_batch = next(iter(train_dataloader))

        # choose a subset of the validation set
        # subset_indices = [0, 1, 2, 3, 4] 
        # subset_valid_set = torch.utils.data.Subset(model.data_module.valid_set, subset_indices)
        ...
        self.valid_batch = next(iter(valid_dataloader))
